...
| Handwerker-Formular (Kontakt): Vorschläge und PLZ | Gewerke-Vorschlag im Feld: `[data-vorschlag]` in `assets/app.js`, Liste `<datalist id="gewerke">` in `quellen/kontakt.html`. Postleitzahl → Ort: `[data-plz]` in `assets/app.js`; `assets/plz.js` ist Bauergebnis von `werkzeuge/plz.py` (DE: GeoNames, CC BY 4.0 – Quellenhinweis im Impressum; AT: Statistik Austria; Quellen und Aufruf im Kopf des Skripts). Schritte: `[data-schritte]` in `assets/app.js`. Weite Kachel ab Schritt 2: `[data-weiten]`; Karte darin: `[data-karte]` in `assets/app.js`, `assets/karte.js` ist Bauergebnis von `werkzeuge/karte.py` (Natural Earth, gemeinfrei) |
| Mail aus dem Handwerker-Formular | `worker/index.js` (Cloudflare-Worker, nur `/api/*`), Versand über die Gmail-API von Google Workspace (Dienstkonto mit domainweiter Delegierung, Bereich `gmail.send`). Empfänger/Absender: `vars` in `wrangler.jsonc`; `GOOGLE_DIENSTKONTO` und `GOOGLE_SCHLUESSEL` nur als Secrets im Cloudflare-Dashboard, nie ins Repo |
