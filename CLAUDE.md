...
| Mail aus dem Handwerker-Formular | `worker/index.js` (Cloudflare-Worker, nur `/api/*`), Versand über Resend. Empfänger/Absender: `vars` in `wrangler.jsonc`; Schlüssel `RESEND_API_KEY` nur als Secret im Cloudflare-Dashboard, nie ins Repo |
